...
HRESULT CScene2D :: Init(D3DXVECTOR3 pos, float width, float height, TEXTURE_TYPE texType, int wblock, int hblock)
{
	...
	pVtx[0].vtx = D3DXVECTOR3(m_vPos.x - m_fWidth * 0.5f		// 中心からの距離なので半分に
...
void CScene2D :: SetVertexPolygon(void)
{
	...
	//--------------------------------------------------------------------------------
	// 描画したい頂点座標の設定(座標はZ字またはN字順に設定する)
	pVtx[0].vtx = D3DXVECTOR3(m_vPos.x + sinf(m_vRot.z - m_fAngle) * m_fLength
							, m_vPos.y - cosf(m_vRot.z - m_fAngle) * m_fLength
							, 0.0f);
	pVtx[1].vtx = D3DXVECTOR3(m_vPos.x + sinf(m_vRot.z + m_fAngle) * m_fLength
							, m_vPos.y - cosf(m_vRot.z + m_fAngle) * m_fLength
							, 0.0f);
	pVtx[2].vtx = D3DXVECTOR3(m_vPos.x - sinf(m_vRot.z + m_fAngle) * m_fLength
							, m_vPos.y + cosf(m_vRot.z + m_fAngle) * m_fLength
							, 0.0f);
	pVtx[3].vtx = D3DXVECTOR3(m_vPos.x - sinf(m_vRot.z - m_fAngle) * m_fLength
							, m_vPos.y + cosf(m_vRot.z - m_fAngle) * m_fLength
							, 0.0f);
	m_pD3DVtxBuff->Unlock();		// ロックしたら必ずアンロック！！
	//********************************************************************************************************************
